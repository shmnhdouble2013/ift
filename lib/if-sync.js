...
function ifSync(opt) {
    ...
    console.log('[if-sync]', opt.savePath);
}
...
function ifInit(opt) {
    ...
    var ifConfigPath = path.join(dataDirPath, 'if-config.js');
    ...
    if (!fs.existsSync(ifConfigPath)) {
        fs.writeFileSync(ifConfigPath, render({
            tplPath: path.join(__dirname, 'tpl/if-config.js.tpl'),
            encoding: encoding
        }), encoding);
    }
